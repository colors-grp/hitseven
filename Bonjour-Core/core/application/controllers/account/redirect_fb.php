...
class redirect_fb extends CI_Controller {
	...
	function index()
	{
		...
		log_message('error', 'redirect_fb: session of user object: ' . $session);
		...
		if($session) {
			...
			if($session) {
				try {
					$me = $facebook->api('/me');
					log_message('error', 'redirect_fb: me: ' . $me);
				} catch (FacebookApiException $e) {
					echo $e->getMessage();
				}
			}
			
			log_message('error', 'redirect_fb: assigned code=1: ' . $me['username']);
			
			// Check if user has connect facebook to a3m
			if ($user = $this->account_facebook_model->get_by_facebook_id($session))
			{
			...
			else
			{
				...
				if ( ! $this->authentication->is_signed_in())
				{
					...
					$this->session->set_userdata('connect_create', array(array('provider' => 'facebook', 'provider_id' => $me['id'], 'provider_UN' => $me['username'], 'provider_email' => $me['email'], 'access_token' => $facebook->getAccessToken()), array('fullname' => $me['name'], 'firstname' => $me['first_name'], 'lastname' => $me['last_name'], 'gender' => $me['gender'], 'dateofbirth' => $me['birthday'],	// not a required field, not all users have it set
